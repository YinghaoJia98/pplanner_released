...
void KD_TREE<PointType>::start_thread()
{
    ...
    pthread_mutex_init(&Rebuild_Ptr_For_Change_And_Use_mutex_lock, NULL);
...
void KD_TREE<PointType>::stop_thread()
{
    ...
    pthread_mutex_destroy(&Rebuild_Ptr_For_Change_And_Use_mutex_lock);
...
void KD_TREE<PointType>::multi_thread_rebuild()
{
    ...
    while (!terminated)
    {
        ...
        // pthread_mutex_lock(&Rebuild_Ptr_For_Change_And_Use_mutex_lock);
        ...
        terminated = termination_flag;
        pthread_mutex_unlock(&termination_flag_mutex_lock);
        // pthread_mutex_unlock(&Rebuild_Ptr_For_Change_And_Use_mutex_lock);
        usleep(100);
    }
                                        ...
                                        bool allow_rebuild, bool is_downsample)
{
    ...
    // 左子树递归删除
    // if ((Rebuild_Ptr == nullptr) || (Rebuild_Ptr == 0x0) || (*root)->left_son_ptr != *Rebuild_Ptr)
    // pthread_mutex_lock(&Rebuild_Ptr_For_Change_And_Use_mutex_lock);
    try
    {
        ...
        if ((Rebuild_Ptr == nullptr) || (Rebuild_Ptr == 0x0)) // || )
        {
        ...
        // 右子树递归删除
        // if ((Rebuild_Ptr == nullptr) || (Rebuild_Ptr == 0x0) || (*root)->right_son_ptr != *Rebuild_Ptr)
        if ((Rebuild_Ptr == nullptr) || (Rebuild_Ptr == 0x0)) // || )
        {
        ...
        {
            if ((*root)->right_son_ptr != *Rebuild_Ptr)
            {
                tmp_counter += Delete_by_range(&((*root)->right_son_ptr), boxpoint, allow_rebuild, is_downsample);
            }
            else
